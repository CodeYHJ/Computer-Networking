...
一个数据包，数据加上头的长度单位用bits表示, 一般长度为1500bytes每个包
...
we saw that using wired ethernet a host

can send it

gigabits per second but over a 3g or 4g

network it may be constrained to a few

megabits per second or less

r is best thought of as a link

transmission rate

but it's sometimes more informally

referred to as the link capacity or

the linked bandwidth if one wants to

send an l-bit packet

into a link at transmission rate r then

the amount of time it takes to send

those bits into the link

is the number of bits to send l divided